...
static void do_cycle(color_ostream& out, int32_t& melt_count, int32_t& trade_count, int32_t& dump_count, int32_t& forbid_count, int32_t& unforbid_count, int32_t& train_count);
...
static void validate_stockpile_configs(color_ostream& out,
    ...
    DEBUG(control, out).print("entering validate_stockpile_configs\n");
    vector<int> to_remove;    
    for (auto& entry : watched_stockpiles) {
        int stockpile_number = entry.first;
        TRACE(control, out).print("validating config of stockpile #%d\n", stockpile_number);
        PersistentDataItem &c = entry.second;
        
        auto bld = find_stockpile(stockpile_number);
        TRACE(control, out).print("find_stockpile(%d): %s", stockpile_number, (!bld ? "true" : "false"));
        TRACE(control, out).print("get_bool(STOCKPILE_CONFIG_MELT): %s\n", (!c.get_bool(STOCKPILE_CONFIG_MELT) ? "true" : "false"));
        TRACE(control, out).print("get_bool(STOCKPILE_CONFIG_TRADE): %s\n", (!c.get_bool(STOCKPILE_CONFIG_TRADE) ? "true" : "false"));
        TRACE(control, out).print("get_bool(STOCKPILE_CONFIG_DUMP): %s\n", (!c.get_bool(STOCKPILE_CONFIG_DUMP) ? "true" : "false"));
        TRACE(control, out).print("get_bool(STOCKPILE_CONFIG_FORBID): %s\n", (!(c.get_int(STOCKPILE_CONFIG_FORBID)>0) ? "true" : "false"));
        TRACE(control, out).print("get_bool(STOCKPILE_CONFIG_TRAIN): %s\n", (!c.get_bool(STOCKPILE_CONFIG_TRAIN) ? "true" : "false"));
        if (!bld || (
            ...
            TRACE(control, out).print("marking stockpile #%d for removal from list of watched stockpiles\n\n", stockpile_number);
            to_remove.push_back(stockpile_number);
            continue;
        }
        TRACE(control, out).print("stockpile #%d validated - placing in the cache for next operation\n\n", stockpile_number);
        cache.emplace(bld, c);
    }
...
DFhackCExport command_result plugin_onupdate(color_ostream &out) {
    ...
    if (world->frame_counter - cycle_timestamp >= CYCLE_TICKS) {
        int32_t melt_count = 0, trade_count = 0, dump_count = 0, forbid_count = 0, unforbid_count = 0, train_count = 0;
        do_cycle(out, melt_count, trade_count, dump_count, forbid_count, unforbid_count, train_count);
        if (0 < melt_count)
        ...
        if (0 < forbid_count)
            out.print("logistics: marked %d item(s) forbidden\n", forbid_count);
        if (0 < unforbid_count)
            out.print("logistics: unforbid %d items(s)\n", unforbid_count);
        if (0 < train_count)
...
class UnforbidStockProcessor : public StockProcessor {
public:
    UnforbidStockProcessor(int32_t stockpile_number, bool enabled, ProcessorStats& stats)
        : StockProcessor("unforbid", stockpile_number, enabled, stats) { }
...
static void scan_stockpile(color_ostream &out, df::building_stockpilest *bld,
        ...
        ForbidStockProcessor &forbid_stockprocessor,
        UnforbidStockProcessor &unforbid_stockprocessor,
    ...
    for (items.begin(bld); !items.done(); ++items) {
        ...
        df::item *item = *items;
        TRACE(cycle,out).print("scan_stockpile: %d  scan item: %d\n", id, item->id);
        ...
        if (item->isAssignedToThisStockpile(id)) {
            ...
            for (df::item *contained_item : contents) {
                ...
                scan_item(out, contained_item, forbid_stockprocessor);
                scan_item(out, contained_item, unforbid_stockprocessor);
            }
            continue;
        }
        scan_item(out, item, melt_stock_processor);
        scan_item(out, item, dump_stock_processor);
        scan_item(out, item, forbid_stockprocessor);
        scan_item(out, item, unforbid_stockprocessor);
...
static void do_cycle(color_ostream& out, int32_t& melt_count, int32_t& trade_count, int32_t& dump_count, int32_t& forbid_count, int32_t& unforbid_count, int32_t& train_count) {
    ...
    ProcessorStats melt_stats, trade_stats, dump_stats, forbid_stats, unforbid_stats, train_stats;
    unordered_map<df::building_stockpilest *, PersistentDataItem> cache;
    ...
    for (auto &entry : cache) {
        ...
        bool unforbid = 2 == c.get_int(STOCKPILE_CONFIG_FORBID);
        ...
        UnforbidStockProcessor unforbid_stock_processor(stockpile_number, unforbid, unforbid_stats);
        ...
        scan_stockpile(out, bld, melt_stock_processor, 
                ...
                forbid_stock_processor, unforbid_stock_processor, 
                train_stock_processor);
    }
    ...
    unforbid_count = unforbid_stats.newly_designated;
...
static int logistics_getStockpileData(lua_State *L) {
    ...
    for (auto bld : df::global::world->buildings.other.STOCKPILE) {
        ...
        UnforbidStockProcessor unforbid_stock_processor(stockpile_number, false, unforbid_stats);
    ...
    stats.emplace("unforbid_designated", unforbid_stats.designated_counts);
    stats.emplace("unforbid_can_designate", unforbid_stats.can_designate_counts);
    ...
    for (auto &entry : cache) {
        ...
        sconfig.emplace("unforbid", unforbid ? "true" : "false");
...
static void logistics_setStockpileConfig(color_ostream& out, int stockpile_number, bool melt, bool trade, bool dump, int forbid, bool train, bool melt_masterworks) {
    DEBUG(control, out).print("entering logistics_setStockpileConfig\n stockpile_number=%d, melt=%d, trade=%d, dump=%d, forbid=%d, train=%d, melt_masterworks=%d\n",
        stockpile_number, melt, trade, dump, forbid, train, melt_masterworks);
...
static int logistics_getGlobalCounts(lua_State *L) {
    ...
    results.emplace("total_unforbid", num_unforbid);
